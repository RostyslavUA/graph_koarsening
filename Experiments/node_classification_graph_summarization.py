    ...
    for seed in run_config['seeds']:
        ...
        for split in range(run_config["splits"]):
            for dataset_name in run_config["datasets"]:
                if hasattr(DATASETS[dataset_name], "rnd_seed"):
                    split = split + seed*run_config["splits"]  # Generate random splits for Cora & Citeseer
                                ...
                                for graph_summarizer_params in parameter_product(params["graph_summarizer_params"][graph_summarizer_name]):
                                    for model_params in parameter_product(params["model_params"][model_name]):
                                        for optimizer_params in parameter_product(params["optimizer_params"][optimizer_name]):
                                            ...
                                            train_graph_summarizer_out_dir = os.path.join(train_graph_summarizer_out_dir, "seed::{}".format(seed))
                                            make_run_dir(train_graph_summarizer_out_dir)
                                            # Save train/val/test masks
                                            train_indcs = dataset[split].ndata['train_mask'].nonzero(as_tuple=True)[0]
                                            val_indcs = dataset[split].ndata['val_mask'].nonzero(as_tuple=True)[0]
                                            test_indcs = dataset[split].ndata['test_mask'].nonzero(as_tuple=True)[0]
                                            torch.save(train_indcs, os.path.join(train_graph_summarizer_out_dir, 'train_indcs.pt'))
                                            torch.save(val_indcs, os.path.join(train_graph_summarizer_out_dir, 'val_indcs.pt'))
                                            torch.save(test_indcs, os.path.join(train_graph_summarizer_out_dir, 'test_indcs.pt'))
                                            ...
                                            out_directory = os.path.join(out_directory, "seed::{}".format(seed))
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        raise RuntimeError("Usage: `python3 Experiments\\node_classification_graph_summarization.py <experiment name>`")
