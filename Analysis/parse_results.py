...
def parse_experiment_recursive(experiment_name: str, path: str, parameters: {}, results_list: [], convergence_list: []):
    """
    ...
    if os.path.exists(os.path.join(path, EXPERIMENT_SUMMARY_FILENAME)):
        ...
        convergence = pd.read_csv(os.path.join(path, TRAINING_CONVERGENCE_FILENAME))
        convergence.columns = convergence.columns.str.strip()
        for parameter in parameters:
            results.loc[0, parameter] = parameters[parameter]
            convergence[parameter] = parameters[parameter]
    ...
    for parameter_setting_dir in os.listdir(path):
        ...
        parse_experiment_recursive(experiment_name, os.path.join(path, parameter_setting_dir),
                                   parameters_copy, results_list, convergence_list)
...
def parse_experiment(experiment_name: str) -> pd.DataFrame:
    ...
    parse_experiment_recursive(experiment_name, os.path.join(RESULTS_BASE_DIR, experiment_name), {}, experiment_results, experiment_convergence)
    if len(experiment_results) != 0:
        experiment_results_frame = pd.concat(experiment_results, ignore_index=True)
        experiment_convergence_frame = pd.concat(experiment_convergence, ignore_index=True)
        return experiment_results_frame, experiment_convergence_frame
    else:
...
def main():
    ...
    for experiment_name in os.listdir(RESULTS_BASE_DIR):
        try:
            experiment_results, experiment_convergence = parse_experiment(experiment_name)
        except ValueError:
            pass
            # print(f"Incomplete experiment {experiment_name}. Skipping")
        experiment_results.to_csv(os.path.join(RESULTS_BASE_DIR, experiment_name, "results.csv"), index=False)
        experiment_convergence.to_csv(os.path.join(RESULTS_BASE_DIR, experiment_name, "convergence.csv"), index=False)
